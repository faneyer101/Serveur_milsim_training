...
_overwrite = -1;
	...
	if (_overwrite != -1) then
	{
		...
		((findDisplay 2550) displayCtrl 2551) ctrlSetStructuredText parseText format["<t align='center'><t size='0.8px'>Voulez-vous vraiment remplacer l'équipement:</t></t><br/><t align='center'><t size='0.7px'>%1</t></t>", _newName];

		((findDisplay 2550) displayCtrl 2552) ctrlSetText "Oui";

		((findDisplay 2550) displayCtrl 2553) ctrlSetText "Non";
		...
		if(lt_prompt_choice) then
		{
			...
			hint format ["%1 sauvegarder", _newName];
			closeDialog 0;
		};
