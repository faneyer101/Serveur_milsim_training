...
_save_use_stand = format ["stand_%1_grenade_main", _number_stand];
_player_equip = format ["player_equipe_stand_%1", _number_stand];
if (missionNamespace getVariable _save_use_stand == 1) then
{
	_formatCursor = format ["cursor_stand_%1", _number_stand];
	_formatCratere = format ["cratere_stand_%1", _number_stand];
