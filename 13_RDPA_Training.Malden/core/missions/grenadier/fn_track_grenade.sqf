...
waitUntil
{
	((getPosATL _grenadeObj) inArea _zone_grenade) && ((getPosATL _grenadeObj) select 2 < 0.1)
};
_grenadePos = getPosATL _grenadeObj;
_distance = _grenadePos distance (getMarkerPos marker_train_grenade_hand);
...
{
	[_number_stand] spawn RDPA_fnc_remove_zone_grenade_main;
};